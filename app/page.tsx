...
export default function Home() {
  ...
  async function confirmarLavagem() {
    ...
    for (const id of lavadoresSel) {
      await supabase.rpc('incrementar_stats_funcionario', {
        p_id_interno: id,
        p_pontos: pontosIndividuais,
        p_comissao: comissaoIndividual
      });
    }
  ...
  const s = { fontFamily: 'sans-serif', maxWidth: 480, margin: '0 auto', padding: 24 };
    ...
    <div style={s}>
        ...
        <div>
          ...
          {servicoSel && (
            <div style={{ background: '#fffbeb', border: '1px solid #C4922A', borderRadius: 10, padding: 12, marginBottom: 12, fontSize: 13 }}>
              <strong>{servicoSel.nome} {servicoSel.preco}€</strong> → Comissão total: <strong style={{ color: '#C4922A' }}>{(servicoSel.pontos * VALOR_PONTO).toFixed(2)}€</strong>
            </div>
          )}
      ...
      {ecrã === 'lavadores' && (
        <div>
          ...
          <div style={{ fontSize: 13, color: '#888', marginBottom: 16 }}>Toca para seleccionar (pode ser mais de 1)</div>
          ...
          {lavadoresSel.length > 0 && servicoSel && (
            <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 10, padding: 12, marginBottom: 12 }}>
              <div style={{ fontSize: 13, marginBottom: 6 }}>💰 Divisão de comissão:</div>
